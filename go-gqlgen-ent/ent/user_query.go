...
type UserQuery struct {
	...
	withLikedArticles *ArticleQuery
...
// QueryLikedArticles chains the current query on the "likedArticles" edge.
func (uq *UserQuery) QueryLikedArticles() *ArticleQuery {
	query := &ArticleQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLikedArticles: uq.withLikedArticles.Clone(),
...
// WithLikedArticles tells the query-builder to eager-load the nodes that are connected to
// the "likedArticles" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithLikedArticles(opts ...func(*ArticleQuery)) *UserQuery {
	query := &ArticleQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withLikedArticles = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLikedArticles; query != nil {
		...
		for i, node := range nodes {
			edgeids[i] = node.ID
			byid[node.ID] = node
			node.Edges.LikedArticles = []*Article{}
		}
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(user.LikedArticlesTable)
			s.Join(joinT).On(s.C(article.FieldID), joinT.C(user.LikedArticlesPrimaryKey[1]))
			s.Where(sql.InValues(joinT.C(user.LikedArticlesPrimaryKey[0]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(user.LikedArticlesPrimaryKey[0]))
		...
		for _, n := range neighbors {
			nodes, ok := nids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "likedArticles" node returned %v`, n.ID)
			}
			for kn := range nodes {
				kn.Edges.LikedArticles = append(kn.Edges.LikedArticles, n)
			}
		}
	}
