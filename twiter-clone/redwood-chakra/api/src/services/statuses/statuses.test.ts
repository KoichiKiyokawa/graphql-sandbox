...
describe('statuses', () => {
  ...
  scenario('creates a status', async (scenario: StandardScenario) => {
    const result = await createStatus({
      input: { content: 'String', accountId: scenario.status.two.accountId },
    })

    expect(result.content).toEqual('String')
    expect(result.accountId).toEqual(scenario.status.two.accountId)
  })
