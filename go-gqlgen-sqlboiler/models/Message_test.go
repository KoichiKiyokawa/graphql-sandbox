...
func testMessageToOneUserUsingToUserIdUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := local.ToUserIdUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadToUserIdUser(ctx, tx, false, (*[]*Message)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ToUserIdUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ToUserIdUser = nil
	if err = local.L.LoadToUserIdUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ToUserIdUser == nil {
		t.Error("struct should have been eager loaded")
...
func testMessageToOneUserUsingFromUserIdUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := local.FromUserIdUser().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := MessageSlice{&local}
	if err = local.L.LoadFromUserIdUser(ctx, tx, false, (*[]*Message)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.FromUserIdUser == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.FromUserIdUser = nil
	if err = local.L.LoadFromUserIdUser(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.FromUserIdUser == nil {
		t.Error("struct should have been eager loaded")
...
func testMessageToOneSetOpUserUsingToUserIdUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetToUserIdUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ToUserIdUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.ToUserIdMessages[0] != &a {
...
func testMessageToOneSetOpUserUsingFromUserIdUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetFromUserIdUser(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.FromUserIdUser != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.FromUserIdMessages[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
...
var (
	messageDBTypes = map[string]string{`ID`: `TEXT`, `Text`: `TEXT`, `FromUserId`: `TEXT`, `ToUserId`: `TEXT`}
	_              = bytes.MinRead
)
