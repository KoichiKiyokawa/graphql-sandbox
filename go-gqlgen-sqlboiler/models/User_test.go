...
func testUserToManyAuthorIdArticles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.AuthorIdArticles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAuthorIdArticles(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthorIdArticles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.AuthorIdArticles = nil
	if err = a.L.LoadAuthorIdArticles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.AuthorIdArticles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyToUserIdMessages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.ToUserIdMessages().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadToUserIdMessages(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ToUserIdMessages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ToUserIdMessages = nil
	if err = a.L.LoadToUserIdMessages(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ToUserIdMessages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyFromUserIdMessages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.FromUserIdMessages().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadFromUserIdMessages(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.FromUserIdMessages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.FromUserIdMessages = nil
	if err = a.L.LoadFromUserIdMessages(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.FromUserIdMessages); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpAuthorIdArticles(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddAuthorIdArticles(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.AuthorIdArticles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.AuthorIdArticles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.AuthorIdArticles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpToUserIdMessages(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddToUserIdMessages(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if first.R.ToUserIdUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.ToUserIdUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.ToUserIdMessages[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ToUserIdMessages[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ToUserIdMessages().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpFromUserIdMessages(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddFromUserIdMessages(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if first.R.FromUserIdUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.FromUserIdUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.FromUserIdMessages[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.FromUserIdMessages[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.FromUserIdMessages().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `TEXT`, `Name`: `TEXT`, `Email`: `TEXT`, `PasswordHash`: `TEXT`}
	_           = bytes.MinRead
)
